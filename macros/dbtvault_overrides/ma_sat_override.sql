{# The dbtvault ma_sat macro is not idempotent due to the latest_records CTE only identfying records by PK rather PK+CDK #}
{# This override compensates for this issue, ensuring idempotence each run #}
...
latest_records AS (
    SELECT {{ dbtvault.prefix(cols_for_latest, 'mas', alias_target='target') }},
           mas.latest_rank,
           DENSE_RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 'mas') }}, {{ dbtvault.prefix([src_cdk], 'mas') }}
                              ORDER BY {{ dbtvault.prefix([src_ldts], 'mas', alias_target='target') }} ASC
           ) AS check_rank
    FROM (
    SELECT {{ dbtvault.prefix(cols_for_latest, 'inner_mas', alias_target='target') }},
           RANK() OVER (PARTITION BY {{ dbtvault.prefix([src_pk], 'inner_mas') }}, {{ dbtvault.prefix([src_cdk], 'inner_mas') }}
...
latest_group_details AS (
    ...
    GROUP BY {{ dbtvault.prefix([src_pk], 'lr') }}, {{ dbtvault.prefix([src_cdk], 'lr') }}, {{ dbtvault.prefix([src_ldts], 'lr') }}
),
...
{% if dbtvault.is_any_incremental() %}
    WHERE EXISTS (
        ...
        AND {{ dbtvault.multikey([src_cdk], prefix=['source_data', 'stage'], condition='=') }}
        AND {{ dbtvault.multikey([src_hashdiff], prefix=['source_data', 'stage'], condition='=') }}
    )
{# endif any_incremental -#}
{%- endif %}
)
