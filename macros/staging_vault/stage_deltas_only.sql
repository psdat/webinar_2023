{%- macro stage_deltas_only(include_source_columns, source_model, hashed_columns, derived_columns, null_columns, ranked_columns, source_ldts) -%}
...
{%- set min_query -%}
    SELECT
        NVL(MAX({{ var('meta_vault_delta_table_max_ldts_field_name') }}), '1900-01-01 00:00:00') AS LATEST_LDTS_FROM_LAST_RUN
    FROM
        {{ target.database ~ var('database_suffix') }}.{{ var('schema_prefix') ~ 'DBT' }}.{{ var('meta_vault_delta_table_name') }}
    WHERE
        {{ var('meta_vault_delta_table_source_field_name') }} = '{{ this | lower }}'
{%- endset -%}

{%- set max_query -%}
    SELECT
        NVL(MAX({{ source_ldts }}), '1900-01-01 00:00:00') AS LATEST_LDTS_FROM_SOURCE_TABLE
    FROM
        {{ source_relation }} 
{%- endset -%}
...
{%- if execute -%}
    ...
    {%- set min_ldts = run_query(min_query).columns[0].values()[0] -%}
    {%- set max_ldts = run_query(max_query).columns[0].values()[0] -%}
...
WITH source_data AS (
    ...
    WHERE
        {{ source_ldts }} > '{{ min_ldts }}'
    AND
        {{ source_ldts }} <= '{{ max_ldts }}'
